...
const Material: React.FC<{
  ...
  baseRotate: number;
  disabled: boolean;
  onLoaded: (config: { color: string; darkMode: boolean; timeout?: boolean }) => void;
  ...
  useEffect(() => {
    ...
    loader.load(data.cover, (texture) => {
  ...
  }, [themeColor, selected, darkMode]);

  const nameTexture = React.useMemo(
    () =>
      createTextTexture(data.name, {
        width: D,
        height: W,
        rotate: true,
        color: textColor,
    ...
    [data.name, textColor, themeColor]
  );
  ...
  // Enable shadow map rendering in the scene
  useEffect(() => {
  ...
  return (
    ...
    >
      <mesh
        castShadow // Object will cast shadows
        receiveShadow // Object will receive shadows
      >
        <boxGeometry args={[D, W, H]} />
        {materials.map((mat, i) => (
...
const MaterialList = ({ list = [], size, gap, rotate, renderContent }: {
  list: any[];
  size: [number, number, number];
  gap?: number;
  rotate?: number;
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      setPositionOffsetX((prev) => Math.max(-list.length * GAP, Math.min(0, prev - (isVertical ? -1 : 1) * e.deltaY * 0.5)));
    };
  ...
  const onPointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    setPositionOffsetX((prev) => Math.max(-list.length * GAP, Math.min(0, prev + (isVertical ? -1 : 1) * deltaX * 10)));
  };
  const onPointerUp = () => {
